      ...
      <main class="relative z-0 flex-1 overflow-y-auto focus:outline-none ">
        <article>
              ...
              <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
                <nav class="-mb-px cursor-pointer flex space-x-8" aria-label="Tabs">
                  <a v-for="tab in tabs" :key="tab.name"
                     ...
                     :aria-current="tab.current ? 'page' : undefined" @click.prevent="setCurrentTab(tab)">{{tab.name}}</a>
                </nav>
              </div>
            </div>
          ...
          <div v-if="tabs[2].current === true">
            <div class="text-sm flex w-full justify-center font-light " > Click to add new cities
              <a  class=" font-medium text-indigo-900 hover:text-indigo-500 underline text-primary-600 hover:underline" ><router-link to="/admin-page">Page Link</router-link></a>
            </div>
          </div>
  ...
  data() {
    return {
      ...
      tabs: [
        ...
        {name: 'Admin page link',href: '#', current:false},
      ],
      favorites: [],
      users: [],
    };
